...
public class DeleteArbitraryElementFromMinHeap {
    ...
    private int findKey(int key, int levelInHeap) {
        ...
        System.out.println("Key : "+key+" | levelInHeap : "+levelInHeap+" | heapArray[levelInHeap] : "+heapArray[levelInHeap]);
        if (levelInHeap>(heapSize-1)/2 ) {
            System.out.println("Not found");
            return -1;
        }
        if (key == heapArray[levelInHeap]) return levelInHeap;
    ...
    private void minHeapify(int pos)
    {
        ...
        if (!isLeaf(pos)) {
            if (heapArray[pos] > heapArray[getleftChild(pos)]
                ...
                // the left child
                if (heapArray[getleftChild(pos)]
                        < heapArray[getRightChild(pos)]) {
